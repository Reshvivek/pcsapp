...
class DonateListView: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    //    var menu: MenuVC = MenuVC()
    override func viewDidLoad() {
        super.viewDidLoad()
        collectionView?.backgroundColor = UIColor.white
        self.tabBarController?.navigationItem.title = "Donation History"
        collectionView?.register(donationCell.self, forCellWithReuseIdentifier: "cellId")
...
class donationCell: UICollectionViewCell {
    ...
    func setupViews() {
        ...
        locationLabel.text = "23 Wiloubhy Street, Epping"
        dateTime.text = "5/11/2017 2:15 pm"
        itemTitle.text = "Used Books"
        thumbnailImage.image = UIImage(named: "furniture")
